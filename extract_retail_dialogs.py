...
print('=' * 80)
print('EXTRACTING DIALOG METHODS FROM retail_pos_screen_modern.dart')
print('=' * 80)
print()

# Find all _show* methods (dialogs, lookups, menus, etc.)
methods_to_extract = []
for i, line in enumerate(lines, 1):
    stripped = line.strip()
    # Match any _show* method
    if re.match(r'^void _show\w+', stripped):
        match = re.match(r'^void\s+(_show\w+)', stripped)
        if match:
...
print(f'Found {len(methods_to_extract)} dialog methods:')
...
for idx, method in enumerate(methods_to_extract):
    ...
    # Find end by looking for next method
...
part_content.append("// Dialog Methods Extension\n")
part_content.append("\n")
part_content.append("part of 'retail_pos_screen_modern.dart';\n")
part_content.append("\n")
part_content.append("extension RetailPOSDialogs on _RetailPOSScreenModernState {\n")
...
print()
print(f'✓ Created lib/screens/retail_pos_screen_modern_dialogs.dart ({len(part_content)} lines)')
print()

# Remove extracted methods from original file (in reverse order)
...
for i, line in enumerate(new_lines):
    if "part 'retail_pos_screen_modern_widgets.dart';" in line:
        new_lines.insert(i + 1, "part 'retail_pos_screen_modern_dialogs.dart';\n")
        break
