#!/usr/bin/env python3
"""Single-pass extraction for setup_screen.dart."""
...
def extract_methods(filepath):
        ...
        if re.search(r'^\s+(Widget|Future<[^>]+>|Future|void|String|int|double|bool|List<[^>]+>|Map<[^>]+>)\s+\w+\([^)]*\)\s*(\basync\b)?\s*\{', line):
            if current_method is None:
                ...
                if match:
                    ...
                    # Skip build() - keep in main file
                    if method_name == 'build':
                        continue
                    # Skip lifecycle methods - keep in main file
                    if method_name in ['initState', 'dispose']:
                        continue
                    current_method = {
                        'name': method_name,
    ...
    # Large widgets (≥100 lines)
    if widget_large:
        part_content = "part of 'setup_screen.dart';\n\n"
        part_content += "extension SetupScreenLargeWidgets on _SetupScreenState {\n"
        for m in widget_large:
            part_content += '\n'.join(m['lines_content']) + '\n\n'
        part_content += "}\n"
        
        part_file = 'lib/screens/setup_screen_large_widgets.dart'
        Path(part_file).write_text(part_content, encoding='utf-8')
        part_files.append(('setup_screen_large_widgets.dart', len(widget_large), sum(m['line_count'] for m in widget_large)))
        print(f"✓ Created large widgets part: {len(widget_large)} methods, {sum(m['line_count'] for m in widget_large)} lines")
    
    # Medium widgets (50-99 lines)
    if widget_medium:
        part_content = "part of 'setup_screen.dart';\n\n"
        part_content += "extension SetupScreenMediumWidgets on _SetupScreenState {\n"
        for m in widget_medium:
            part_content += '\n'.join(m['lines_content']) + '\n\n'
        part_content += "}\n"
        
        part_file = 'lib/screens/setup_screen_medium_widgets.dart'
        Path(part_file).write_text(part_content, encoding='utf-8')
        part_files.append(('setup_screen_medium_widgets.dart', len(widget_medium), sum(m['line_count'] for m in widget_medium)))
    ...
    if widget_small:
        part_content = "part of 'setup_screen.dart';\n\n"
        part_content += "extension SetupScreenSmallWidgets on _SetupScreenState {\n"
        for m in widget_small:
            part_content += '\n'.join(m['lines_content']) + '\n\n'
        part_content += "}\n"
        
        part_file = 'lib/screens/setup_screen_small_widgets.dart'
        Path(part_file).write_text(part_content, encoding='utf-8')
        part_files.append(('setup_screen_small_widgets.dart', len(widget_small), sum(m['line_count'] for m in widget_small)))
        print(f"✓ Created small widgets part: {len(widget_small)} methods, {sum(m['line_count'] for m in widget_small)} lines")
    
    # Future methods
    if future_methods:
        part_content = "part of 'setup_screen.dart';\n\n"
        part_content += "extension SetupScreenFutures on _SetupScreenState {\n"
        for m in future_methods:
            part_content += '\n'.join(m['lines_content']) + '\n\n'
        part_content += "}\n"
        
        part_file = 'lib/screens/setup_screen_futures.dart'
        Path(part_file).write_text(part_content, encoding='utf-8')
        part_files.append(('setup_screen_futures.dart', len(future_methods), sum(m['line_count'] for m in future_methods)))
        print(f"✓ Created futures part: {len(future_methods)} methods, {sum(m['line_count'] for m in future_methods)} lines")
    
    # Void methods
    if void_methods:
        part_content = "part of 'setup_screen.dart';\n\n"
        part_content += "extension SetupScreenOperations on _SetupScreenState {\n"
        for m in void_methods:
            part_content += '\n'.join(m['lines_content']) + '\n\n'
        part_content += "}\n"
        
        part_file = 'lib/screens/setup_screen_operations.dart'
        Path(part_file).write_text(part_content, encoding='utf-8')
        part_files.append(('setup_screen_operations.dart', len(void_methods), sum(m['line_count'] for m in void_methods)))
        print(f"✓ Created operations part: {len(void_methods)} methods, {sum(m['line_count'] for m in void_methods)} lines")
    
    # Helper methods
    if helper_methods:
        part_content = "part of 'setup_screen.dart';\n\n"
        part_content += "extension SetupScreenHelpers on _SetupScreenState {\n"
        for m in helper_methods:
            part_content += '\n'.join(m['lines_content']) + '\n\n'
        part_content += "}\n"
        
        part_file = 'lib/screens/setup_screen_helpers.dart'
        Path(part_file).write_text(part_content, encoding='utf-8')
        part_files.append(('setup_screen_helpers.dart', len(helper_methods), sum(m['line_count'] for m in helper_methods)))
...
if __name__ == '__main__':
    extract_methods('lib/screens/setup_screen.dart')
